...
import { apiFetch, type PagedResponse, type VehicleListing } from "@/lib/api";
...
export default function VehiclesPage() {
  ...
  async function search() {
    ...
    try {
      ...
      const res = await apiFetch<PagedResponse<VehicleListing>>(`/api/vehicles${qs}`);
      setItems(res.items);
    } catch (e: any) {
  ...
  const rows = items.map((x) => [
    <a key={x.listingHash} href={x.url} target="_blank" rel="noreferrer">
      {x.year ?? ""} {x.make ?? ""} {x.model ?? ""} {x.modelConfig ?? ""}
    </a>,
  ...
  return (
    <Container>
      <h1 className="text-xl font-semibold">Vehicle Listings</h1>

      <div className="mt-4 grid gap-3 rounded border border-slate-200 p-4 md:grid-cols-4">
        <input className="rounded border border-slate-300 px-3 py-2" placeholder="Make (e.g., toyota)"
          value={make} onChange={(e) => setMake(e.target.value)} />
        <input className="rounded border border-slate-300 px-3 py-2" placeholder="Model (e.g., camry)"
      ...
      <div className="mt-4">
        <Table
          headers={["Listing", "Price", "Mileage", "Location", "Dealer", "Scraped"]}
          rows={rows}
        />
      </div>
    </Container>
